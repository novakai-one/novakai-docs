...
export function TopBar(p: TopBarProps) {
  return (
    <header className="flex shrink-0 flex-col border-b">
      {/* Row 1 — identity + file/global actions. */}
      ...
      <div className="min-w-0 flex-1">
        ...
        {p.subtitle && (
          <div className="truncate text-xs text-muted-foreground" title={p.subtitle}>
            {p.subtitle}
          </div>
        )}
      </div>
      ...
      {/* Row 2 — full-width section nav. Wraps instead of clipping, so every tab
          is reachable at any width with the sidebar open or closed. */}
      <nav className="flex flex-wrap items-center gap-0.5 border-t px-3 py-1.5">
        <Button
